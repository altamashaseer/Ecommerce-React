...
const Sort = () => {
  const { filter_products, grid_view, setGridView, setListView, sorting, setSortingOption } = useFilterContext();

  const handleSortingChange = (event) => {
    const selectedOption = event.target.value;
    setSortingOption(selectedOption); // Pass the selected option directly
  };
